...
// ========== FIREBASE ==========
async function saveToCloud() { if(!currentUser) return; await db.ref(`users/${currentUser.uid}/items`).set(items); await db.ref(`users/${currentUser.uid}/settings`).set(userSettings); }
async function loadUserData(uid) { showLoading(); const snapItems = await db.ref(`users/${uid}/items`).once('value'); items = snapItems.val() || []; if(!Array.isArray(items)) items = []; const snapSettings = await db.ref(`users/${uid}/settings`).once('value'); const settings = snapSettings.val(); if(settings) { userSettings = settings; if(!userSettings.categories) userSettings.categories = ['Alimentação', 'Transporte', 'Lazer', 'Saúde', 'Moradia']; } else { userSettings = { meta: 0, categories: ['Alimentação', 'Transporte', 'Lazer', 'Saúde', 'Moradia'] }; } updateCategoryDatalist(); updateDescriptionDatalist(); updateMetaUI(); document.getElementById('syncStatus').textContent = `✅ ${items.length} itens carregados`; document.getElementById('syncStatus').classList.add('synced'); refreshAllUI(); hideLoading(); }
...
function updateMetaUI() {
    ...
    document.getElementById('metaValueDisplay').innerHTML = `R$ ${meta.toLocaleString('pt-BR', {minimumFractionDigits: 2})}`;
    if(meta === 0) { document.getElementById('progressBar').style.width = '0%'; document.getElementById('metaStatus').innerHTML = 'Defina uma meta para começar'; return; }
    const monthExpense = getMonthStats().expense;
    const percent = Math.min(100, (monthExpense / meta) * 100);
    document.getElementById('progressBar').style.width = `${percent}%`;
    if(percent >= 100) document.getElementById('metaStatus').innerHTML = '⚠️ Atenção: gastos excederam a meta!';
    else document.getElementById('metaStatus').innerHTML = `Gastos deste mês: ${percent.toFixed(0)}% da meta`;
}
...
function saveMeta() {
    const val = parseFloat(document.getElementById('metaAmountInput').value);
    if(!isNaN(val) && val >= 0) { userSettings.meta = val; if(currentUser) saveToCloud(); updateMetaUI(); closeMetaModal(); }
...
// ========== FUNÇÕES DE DADOS (já existentes, mantidas) ==========
...
// ========== EVENTOS ==========
auth.onAuthStateChanged(async (user)=>{ if(user){ currentUser=user; document.getElementById('authSection').style.display='none'; document.getElementById('userInfo').style.display='flex'; document.getElementById('userEmail').textContent=user.email; await loadUserData(user.uid); } else { currentUser=null; items=[]; userSettings = { meta:0, categories:['Alimentação','Transporte','Lazer','Saúde','Moradia'] }; document.getElementById('authSection').style.display='flex'; document.getElementById('userInfo').style.display='none'; document.getElementById('syncStatus').textContent='🔐 Faça login para salvar'; document.getElementById('syncStatus').classList.remove('synced'); refreshAllUI(); } });

document.getElementById('financialForm').addEventListener('submit',(e)=>{ e.preventDefault(); const category = document.getElementById('category').value.trim(); const finalCategory = category === '' ? 'Sem categoria' : category; items.push({ id:editingItemId||Date.now(), type:document.getElementById('type').value, title:document.getElementById('description').value, category:finalCategory, amount:parseFloat(document.getElementById('amount').value), status:document.getElementById('status').value, date:formatDateKey(selectedDate) }); saveData(); closeModal(); });
...
window.onclick=(e)=>{ if(e.target===document.getElementById('modal')) closeModal(); if(e.target===document.getElementById('reportModal')) closeReportModal(); if(e.target===document.getElementById('metaModal')) closeMetaModal(); };

// Modo escuro
const darkModeToggle = document.getElementById('darkModeToggle');
if(localStorage.getItem('darkMode') === 'enabled') document.body.classList.add('dark-mode');
darkModeToggle.addEventListener('click', () => { document.body.classList.toggle('dark-mode'); localStorage.setItem('darkMode', document.body.classList.contains('dark-mode') ? 'enabled' : 'disabled'); });

// Expor funções globais
window.login = login; window.signup = signup; window.logout = logout; window.openNewTransaction = openNewTransaction; window.editTransaction = editTransaction; window.closeModal = closeModal; window.deleteItem = deleteItem; window.openFullReport = openFullReport; window.generateWeeklyReport = generateWeeklyReport; window.generateMonthlyReport = generateMonthlyReport; window.closeReportModal = closeReportModal; window.prevMonth = prevMonth; window.nextMonth = nextMonth; window.selectDate = selectDate; window.editMeta = editMeta; window.saveMeta = saveMeta; window.closeMetaModal = closeMetaModal; window.addNewCategory = addNewCategory;
